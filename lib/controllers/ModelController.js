			...
			getSelf().beforeUpdate(req, res, function(){
				...
				for(var i in req.body){
					if( i!="_id" && i != "_metadata")
					{
						...
						if(req.record[i] instanceof Array){
							req.record.save(function(err){
								if(err){
									res.json(500,{error:err});
								}
							});
							// var tmp = req.record.toJSON();
							// for(var arrayIndex in tmp[i]){
								
							// 	if( req.body[i].hasOwnProperty(arrayIndex) && 
							// 		req.body[i][arrayIndex].hasOwnProperty('_id') && 
							// 		req.record[i].id(req.body[i][arrayIndex]._id) ){
							// 		for(var recordIndex in req.body[i][arrayIndex]){
							// 			if(recordIndex!='_id')
							// 				req.record[i][arrayIndex].set(recordIndex, req.body[i][arrayIndex][recordIndex]);
							// 		}
							// 		req.record.save();
							// 	}else{
							// 		req.record[i].id(tmp[i][arrayIndex]._id).remove();
							// 		req.record.save();
							// 	}
							// }
							// for(var arrayIndex in req.body[i]){
							// 	if(!req.body[i][arrayIndex].hasOwnProperty('_id')  && !req.record[i].id(req.body[i][arrayIndex]._id) ){
							// 		req.record[i].push(req.body[i][arrayIndex]);
							// 		req.record.save();
							// 	}
							// }
						}else{
