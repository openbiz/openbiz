...
module.exports = function(app, name){
	var controller = require("../objects/Controller").extend({
		...
		update:function(req,res){
			...
			for(var i in req.body){
				if( i!="_id" && i != "_metadata")
					...
					req.record["_metadata"].state = req.body["_metadata"].state;
				}
			}
		...
		processChangeLog:function(req,action){
			if(typeof app.getModel.call(app,getSelf()._model).schema.features != "undefined"){
				if(app.getModel.call(app,getSelf()._model).schema.features.enableLogDataChange == false){
					return;
				}
			}
			var updateLog = {
			...
			for(var i in req.body){
				if(!_.isArray(req.body[i])&&!_.isEqual(req.record[i], req.body[i]&&i!="_metadata")){
