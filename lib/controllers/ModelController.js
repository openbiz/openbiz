...
module.exports = function(app, name){
	...
	//			query=rr&page=1&per_page=3&total_pages=0&total_entries=0&sort_by=expiredDate&order=desc
			...
			var queryString;
			if(req.query.query){
				queryString = {name:req.query.query};
			}

			var async = require('async');
			async.series({
				count:function(cb){
					var query = model.find(queryString).count(function(err,count){
						cb(err,count);
					});
				},items:function(cb){
					...
					var query = model.find(queryString).skip(parseInt(page * pageSize)).limit(pageSize);
					if(sort && order){
		...
		update:function(req,res){
			...
			req.record.update(req.body,function(err,recordNew){
				if(err){
					res.json(500,{error:err});
				}
				else{				
					req.recordNew = recordNew;
