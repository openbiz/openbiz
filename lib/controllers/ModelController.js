...
module.exports = function(app, name){
	...
	//			query=rr&page=1&per_page=3&total_pages=0&total_entries=0&sort_by=expiredDate&order=desc
		...
		update:function(req,res){
			...
			getSelf().processChange(req,"UPDATE");
		...
		processChange:function(req,action){
			var updateLog = {
				id:req.user.id,
				original:{},
				current:{}
			...
			for(var i in req.body){
				if(!_.isEqual(req.record[i], req.body[i])){
					updateLog.original[i] = req.record[i];
					updateLog.current[i] = req.body[i];
				}
			}
			if(!_.isEmpty(updateLog.original)){
				if(_.isUndefined(req.record._metadata)){
					req.record._metadata = {};
					req.record._metadata.updates = [];
				}
